...
FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const FInv_ItemManifest& InItemManifest)
{
	...
	for (const auto& GridSlot : GridSlots)
	{
		...
		if (!HasRoomAtIndex(GridSlot, GetItemDimensions(InItemManifest), CheckedIndices, TentativelyClaimed, InItemManifest.GetItemType(),MaxStackAmount))
		{
			continue;
		}
	...
	const int32& MaxStackAmount)
{
	...
	UInv_InventoryStatics::ForEach2D(
		...
		{
			if (CheckForSlotConstraints(GridSlot, SubGridSlot, CheckedIndices, OutTentativelyClaimed, ItemType, MaxStackAmount))
			{
...
bool UInv_InventoryGrid::CheckForSlotConstraints(
	UInv_GridSlot* GridSlot,
	UInv_GridSlot* SubGridSlot,
	const TSet<int32>& CheckedIndices,
	TSet<int32>& OutTentativelyClaimed,
	...
	const int32& MaxStackAmount)
{
	...
	//checks if this grid slot is an upper left slot?
	...
	if (!DoesItemTypeMatch(SubGridSlot->GetInventoryItem().Get(), ItemType)) return false;
...
bool UInv_InventoryGrid::DoesItemTypeMatch(UInv_InventoryItem* SubItem, const FGameplayTag& ItemType) const
{
	return SubItem->GetItemManifest().GetItemType().MatchesTagExact(ItemType);
}

bool UInv_InventoryGrid::IsItemInGridBounds(const int32& StartIndex, const FIntPoint& ItemDimensions) const
{
	if (StartIndex >= GridSlots.Num() || StartIndex < 0) return false;
...
void UInv_InventoryGrid::OnSlottedItemClicked(int32 GridIndex, const FPointerEvent& InMouseEvent)
{
	...
	if (!IsValid(HoverItem) &&  LeftMouseClick(InMouseEvent))
	{
		PickUpSlottedItem(Item, GridIndex);
	}
}
...
void UInv_InventoryGrid::AssignHoverItem(UInv_InventoryItem* Item)
{
	...
	HoverItem->SetImageIcon(IconBrush);
...
void UInv_InventoryGrid::RemoveItemFromGrid(UInv_InventoryItem* Item, int32 GridIndex)
{
	...
	UInv_InventoryStatics::ForEach2D(GridIndex, Columns, GridFragment->GetGridSize(), GridSlots, [&](UInv_GridSlot* GridSlot)
	{
		GridSlot->SetInventoryItem(nullptr);
		GridSlot->SetUpperLeftIndex(INDEX_NONE);
		GridSlot->SetIsAvailable(true);
		GridSlot->SetStackAmount(0);
		GridSlot->SetSlotToUnoccupied();
	});

	if (SlottedItems.Contains(GridIndex))
	{
		UInv_SlottedItem* FoundSlottedItem;
